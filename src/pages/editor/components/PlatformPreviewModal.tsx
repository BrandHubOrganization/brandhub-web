...
const PLATFORM_ICONS: Record<PlatformType, React.ReactNode> = {
  FACEBOOK: <Share2 className="h-4 w-4 text-blue-500" />,
  INSTAGRAM: <Globe className="h-4 w-4 text-pink-500" />,
  TIKTOK: <Video className="h-4 w-4 text-slate-800 dark:text-slate-200" />,
  THREADS: <AtSign className="h-4 w-4 text-slate-700 dark:text-slate-300" />,
  YOUTUBE: <MessageSquare className="h-4 w-4 text-red-500" />,
};
...
export const PlatformPreviewModal: React.FC<PlatformPreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
      <div className="animate-in fade-in zoom-in-95 flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl duration-200 dark:border-slate-800 dark:bg-slate-900">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 p-4 dark:border-slate-800">
          <div className="flex items-center gap-2">
            <Eye className="h-5 w-5 text-[#f05a28]" />
