...
export const MediaDetailPanel: React.FC<MediaDetailPanelProps> = ({ media, onClose, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/40 backdrop-blur-xs flex justify-end transition-opacity">
      <div className="w-full max-w-md bg-card h-full shadow-2xl flex flex-col transform transition-transform border-l border-border">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          {/* Media Preview Box */}
          <div className="rounded-xl overflow-hidden bg-zinc-950 border border-border relative group flex items-center justify-center min-h-[220px]">
            {media.type === 'video' ? (
              <video
                src={media.url}
                controls
          ...
          <div>
            ...
            <h4 className="font-medium text-foreground text-base break-all leading-snug">
              {media.filename}
            </h4>
          </div>

          {/* Metadata Grid */}
          <div className="bg-muted/50 rounded-xl p-4 space-y-3 text-sm text-zinc-600 dark:text-zinc-300 border border-zinc-100 dark:border-zinc-800">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-muted-foreground">S3 Direct URL</label>
            <div className="flex gap-2">
              <input
                type="text"
                readOnly
                value={media.url}
                className="flex-1 text-xs font-mono bg-muted border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-zinc-600 dark:text-zinc-300 focus:outline-hidden"
              />
              <button
        ...
        {/* Footer Actions */}
        <div className="p-4 border-t border-border bg-zinc-50 dark:bg-zinc-900 flex justify-between items-center">
          <button
