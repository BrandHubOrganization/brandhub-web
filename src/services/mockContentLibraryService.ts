...
const INITIAL_MEDIA_ITEMS: MediaItem[] = [
  ...
  },
  {
    ...
    thumbnailUrl: 'https://images.unsplash.com/photo-1509042239860-f550ce710b93?auto=format&fit=crop&w=600&q=80',
    type: 'video',
    sizeBytes: 12400000,
    width: 1080,
    height: 1920,
...
const INITIAL_HASHTAG_GROUPS: HashtagGroup[] = [
  {
    ...
    tags: ['#fashion', '#summer2026', '#styleinspo', '#ootd', '#lookbook', '#brandhub'],
  ...
  },
  {
    ...
    workspaceId: 'ws-default',
    name: 'Tech & AI Trends',
    tags: ['#technews', '#aidesign', '#marketingtech', '#saas', '#futureofwork', '#creators'],
  ...
  },
  {
    ...
    tags: ['#coffeetime', '#morningvibes', '#aesthetic', '#lifestyle', '#relax', '#worklife'],
...
const INITIAL_POST_TEMPLATES: PostTemplate[] = [
  {
    ...
    caption: 'Chào đón bộ sưu tập Mùa Hè 2026 rực rỡ! Khám phá ngay những thiết kế độc quyền giúp bạn nổi bật bất chấp cái nắng hè. Ghé thăm gian hàng online của chúng mình để nhận ưu đãi lên đến 30%!',
  ...
  },
  {
    ...
    caption: 'Tái định nghĩa trải nghiệm quản lý truyền thông với sự hỗ trợ từ AI! Chúng tôi vừa ra mắt tính năng Content Library giúp tối ưu hóa 50% thời gian sáng tạo bài viết.',
  ...
  },
  {
    ...
    caption: 'Một tách cà phê nóng khởi đầu ngày mới làm việc tràn đầy năng lượng. Hãy tạo cho mình một khoảng lặng nghỉ ngơi và tận hưởng những điều nhỏ bé.',
...
class MockContentLibraryService {
  ...
  // --- MEDIA ---
  async getMedia(params: MediaQueryParams): Promise<PaginatedResponse<MediaItem>> {
  ...
  async createHashtagGroup(name: string, tags: string[]): Promise<HashtagGroup> {
    await this.delay(400);
  ...
  async updateHashtagGroup(id: string, name: string, tags: string[]): Promise<HashtagGroup> {
    await this.delay(300);
  ...
  async createTemplate(title: string, caption: string, hashtagGroupId?: string): Promise<PostTemplate> {
    await this.delay(400);
