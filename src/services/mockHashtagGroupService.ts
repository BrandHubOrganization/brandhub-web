import type { HashtagGroup, CreateHashtagGroupPayload } from '@/types/hashtagGroup';

const INITIAL_GROUPS: HashtagGroup[] = [
  {
    id: 'hg-1',
    name: 'Chiến Dịch Mùa Hè 2026 ☀️',
    hashtags: ['#SummerVibes', '#Fashion2026', '#Lookbook', '#SunLight', '#BrandHub'],
    count: 5,
  ...
  },
  {
    ...
    hashtags: ['#TechNews', '#AIContent', '#FutureTech', '#SmartTools', '#Saas'],
    count: 5,
  ...
  },
  {
    ...
    hashtags: ['#Sneakerhead', '#AirMaxPulse', '#NikeAir', '#StreetStyle', '#KicksOfTheDay'],
    count: 5,
  ...
  },
  {
    ...
    hashtags: ['#CoffeeVibes', '#Lifestyle', '#MorningRoutine', '#Aesthetic', '#ChillVibes'],
    count: 5,
...
class MockHashtagGroupService {
  ...
  async createGroup(payload: CreateHashtagGroupPayload): Promise<HashtagGroup> {
    ...
    if (this.groups.some((g) => g.name.toLowerCase() === payload.name.toLowerCase())) {
  ...
  async updateGroup(id: string, payload: CreateHashtagGroupPayload): Promise<HashtagGroup> {
    await this.delay(300);

    // Validate unique name excluding current id
    if (
      this.groups.some(
        (g) => g.id !== id && g.name.toLowerCase() === payload.name.toLowerCase()
      )
    ) {
      throw new Error('DUPLICATE_NAME');
    }
